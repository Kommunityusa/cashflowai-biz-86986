...
interface Rule {
  ...
  user_id: string;
  rule_name: string;
  condition_field: 'description' | 'vendor_name' | 'amount';
  condition_operator: 'contains' | 'equals' | 'greater_than' | 'less_than';
  ...
  const fetchRules = async () => {
    try {
      ...
      const { data, error } = await supabase
        ...
        .select('*, categories:action_category_id(name, color)')
        .eq('user_id', user.id)
        ...
        for (const rule of activeRules) {
          ...
          switch (rule.condition_operator) {
            ...
            case 'equals':
              ...
              break;
            case 'greater_than':
              matches = parseFloat(transaction[rule.condition_field]) > parseFloat(rule.condition_value);
              break;
            case 'less_than':
              matches = parseFloat(transaction[rule.condition_field]) < parseFloat(rule.condition_value);
              break;
                ...
                <div className="grid grid-cols-3 gap-2">
                  <div>
                    ...
                    <Select
                      ...
                      onValueChange={(value: any) => setNewRule({ ...newRule, condition_field: value })}
                    >
                      <SelectTrigger>
                        <SelectValue />
                      </SelectTrigger>
                  ...
                  <div>
                    ...
                    <Select
                      ...
                      onValueChange={(value: any) => setNewRule({ ...newRule, condition_operator: value })}
                    >
                      <SelectTrigger>
                        <SelectValue />
                      </SelectTrigger>
